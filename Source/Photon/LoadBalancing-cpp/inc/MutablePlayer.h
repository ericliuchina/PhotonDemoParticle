...
namespace ExitGames
{
	namespace LoadBalancing
	{
		...
		public:
			...
			template<typename ktype, typename vtype> void addCustomProperty(const ktype& key, vtype pValueArray, typename Common::Helpers::ArrayLengthType<vtype>::type arrSize, const WebFlags& webflags=WebFlags());
			template<typename ktype, typename vtype> void addCustomProperty(const ktype& key, vtype pValueArray, const short* pArrSizes, const WebFlags& webflags=WebFlags());
		...
		template<typename ktype, typename vtype>
		void MutablePlayer::addCustomProperty(const ktype& key, vtype pValueArray, typename Common::Helpers::ArrayLengthType<vtype>::type arrSize, const WebFlags& webflags)
		{
			Common::Hashtable hash;
		...
		template<typename ktype, typename vtype>
		void MutablePlayer::addCustomProperty(const ktype& key, vtype pValueArray, const short* pArrSizes, const WebFlags& webflags)
		{
			Common::Hashtable hash;
