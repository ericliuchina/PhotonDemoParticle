...
{
public:
	PeerStatesStrChecker(){assert(sizeof(PeerStatesStr)/sizeof(const char*) == PeerStates::Disconnected + 1);}
} checker;
...
{
	if(x >= 0 && x < gridSize && y >= 0 && y < gridSize && (localPlayer.x != x || localPlayer.y != y))
	{
		localPlayer.x = x;
		localPlayer.y = y;
		Hashtable data;
